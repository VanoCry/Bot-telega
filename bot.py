...
#возвращение в меню
    ...
    if message.text == '🤩аккаунты🤩':
        markup.add(itemback)
        bot.send_message(message.chat.id, "⚠️Раздел находится на доработке😔⚠️", reply_markup=markup)
        wrong_png = open('wrong.tgs', 'rb')
        bot.send_sticker(message.chat.id, wrong_png)
    if message.text == '🎒Кейсы🎒':
        shirp = types.KeyboardButton('Кейс 🎇"ШИРП"🎇')
        royal = types.KeyboardButton('Кейс 👑"ROYAL"👑')
        markup.add(royal,shirp)
        markup.add(itemback)
